...
# CA.  Access control for the private keys is platform-specific and layered on
# by the per-platform tls-leaf modules; the agenix defaults (root-owned, 0400)
# apply otherwise.
################################################################################
{
  config,
...
in
{
  ...
  config = mkIf cfg.enable (
    ...
    in
    {
      ...
      # Contribute per-leaf expiry checks unconditionally.  goss only runs when
      # "goss" is in networking.monitors (see nixos-modules/goss.nix), so
      # leaving these laid down on non-monitored hosts is inert.
