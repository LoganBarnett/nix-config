...
~remote-deploy~ is structurally incompatible with deploys driven from the
private repository: it evaluates the flake on the target as root, and root
there has no credential to fetch the private lens's ~git+ssh://gitea.proton~
flake inputs.  Use ~proton-deploy~ for those, which keeps evaluation on the
workstation where the credentials live.
...
The full UEFI/PC bootstrap runbook, from disk partitioning through the first
agenix rekey, lives at [[file:bootstrap/bootstrap.org][bootstrap/bootstrap.org]].
...
This generates the host's real SSH keypair up front: the public half lands in
~secrets/$host-pub-key.pub~, while the private half is kept encrypted to the
master key as ~secrets/$host-host-key.age~ (requires the committed master public
half - see [[file:secrets.org][secrets.org]] for the one-time setup).  Commit the new and changed
files under ~secrets/~ before deploying.

Once that's complete, use ~image-deploy~ thusly, with your SD card plugged in:

#+begin_src shell-script :results none :exports code
image-deploy --host $host --image tmp/images/host/sd-image/*.img.zst
#+end_src
...
#+begin_src shell-script :results none :exports code
caffeinate proton-deploy switch $host
#+end_src

If secrets are missing on the freshly booted host (an empty ~/run/agenix~),
see the triage steps in [[file:bootstrap/bootstrap.org::*Bootstrap Pis][bootstrap.org → Bootstrap Pis]].
...
Raspberry Pi should see [[rpi: initial deployment steps]] instead.
...
~makemkv~, ~opencode~, ~opencode-bun~, ~signal-desktop-bin~, ~stats~,
~yt-dlp~, ~zoom-us~.
...
Tokens a third-party service issues (API keys, personal access tokens) can't be
generated, so they live under ~secrets/issued/~ as
~<service>-<purpose>-<host-id>.age~, one per consuming host so that revoking a
token never reaches past that host.  Store a freshly issued one with:

#+begin_src sh :results none :exports code
agenix edit secrets/issued/gitea-mcp-scandium.age
agenix rekey generate --rekey --add-to-git
#+end_src
...
A separate tether failure — Nix-built c-ares clients (curl, git,
nix-daemon) unable to use the tether's scoped link-local nameserver — has a
durable resolver-layer fix: the ~services.dns-c-ares-scopeless-fix~
darwin-module runs a local dnsmasq that all DNS routes through.  Design,
failure taxonomy, and the deferred filter-AAAA reconciler are in
[[file:docs/tether-dns.org][docs/tether-dns.org]].

** Claude Code leaving the Mac thrashing

Symptom: many ~.claude-wrapped~ / ~claude bg-spare~ processes in ~ps~, swap
climbing, everything sluggish, long after the Claude session that started
them is gone.

Cause is Claude Code not reaping what it spawns: Bash tool shells are their
own session leaders and survive their session, scratchpad-launched servers
have no parent link at all, and the transient daemon leaks its pre-warmed
spares.  ~claude-dislodge~ reports every Claude-related process and, with
~--execute~, kills the orphans and the daemon while leaving live sessions
alone.  Process taxonomy, the macOS ~ps~ gotcha, and the procedure are in
[[file:docs/claude-code-runaway.org][docs/claude-code-runaway.org]].

** Firefox refusing to start after a deploy

Symptom: Firefox on the Mac exits with "Firefox couldn't load your profile"
right after a ~proton-deploy switch~ that rebuilt ~firefox-bin~, even at the
same version.

Cause is Firefox keying per-installation profiles on its store path and then
failing to record the new installation in the home-manager-owned, read-only
~profiles.ini~.  The launchd agent in ~home-configs/firefox.nix~ sets
~MOZ_LEGACY_PROFILES~ so that bookkeeping is skipped.  What to check, the
emergency workaround, and the leftovers are in
[[file:docs/firefox-profile-load-failure.org][docs/firefox-profile-load-failure.org]].
